...
type ApiListResponse<T> = {
  ...
  message: string;
  data: T[];
  meta: { total: number; page: number; limit: number; totalPages: number; category: string };
};
...
@Component({
  ...
  template: `
    <section class="container mx-auto p-4">
      <h1 class="text-2xl font-semibold mb-4">Categoría: {{ slug() }}</h1>
      @if (posts()) {
...
export class CategoryComponent {
  ...
  combineLatest([this.slug$, this.page$]).pipe(
      switchMap(([slug, p]) =>
  this.http.get<ApiListResponse<PostListItem> | PostListItem[]>(`/api/category/${slug}`, {
  ...
  readonly meta = computed(() => {
    ...
    return r?.meta ?? { total: 0, page: 1, limit: this.limit, totalPages: 1, category: this.slug() };
  });
