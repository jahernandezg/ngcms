...
@Component({
  ...
  template: `
  <nav class="bg-white/80 dark:bg-gray-900/70 backdrop-blur border-b border-gray-200 dark:border-gray-700 shadow-sm mb-4 transition-colors" role="navigation" aria-label="Main">
      <div class="container mx-auto px-4 py-2 flex flex-wrap items-center gap-4">
        ...
        <ul class="flex flex-wrap gap-4" *ngIf="items().length; else loadingTpl" role="menubar">
          ...
          <a *ngIf="isInternal(item); else extTpl"
            ...
            [queryParams]="queryParamsFor(item)"
                 ...
                 >{{ item.title }}</a>
              <ng-template #extTpl>
                <a [href]="item.url" class="hover:text-blue-600 transition-colors" role="menuitem" [attr.target]="item.openNewWindow? '_blank': null" rel="noopener noreferrer">{{ item.title }}</a>
              </ng-template>
              <ul *ngIf="item.children?.length"
                  class="absolute left-0 top-full bg-white border shadow-lg rounded min-w-[10rem] z-20 focus:outline-none"
                  [class.hidden]="openSubmenu() !== item.id"
                  role="menu"
                  [attr.aria-label]="item.title">
                <li *ngFor="let c of item.children; let ci = index" class="px-4 py-2 hover:bg-gray-50" role="none">
        ...
        <button type="button" class="ml-auto text-sm px-2 py-1 border rounded" (click)="toggleDark()" [attr.aria-pressed]="theme.darkMode()">{{ theme.darkMode() ? 'Light' : 'Dark' }}</button>
        <ng-template #loadingTpl>
          <span class="text-sm text-gray-500">Cargando menú…</span>
        </ng-template>
      </div>
    </nav>
  `,
