              ...
              <li class="relative" [class.has-children]="item.children?.length" role="none">
                ...
                @if (item.children?.length) {
                  <ul
                    ...
                    @for (c of item.children; let ci = $index; track c.id) {
                      <li class="px-4 py-2 hover:bg-[var(--color-surface-alt)]" role="none">
                        @if (isInternal(c)) {
                          <a [routerLink]="linkFor(c)" [queryParams]="queryParamsFor(c)" class="block w-full" role="menuitem" tabindex="-1">{{ c.title }}</a>
                        } @else {
